"""
After Hours Detection Alert Service using Twilio
Sends SMS alerts when a person is detected after hours with image and metadata
"""
...
class AfterHoursDetectionAlerts:
    """Service for sending after-hours detection alerts via Twilio"""
    ...
    def __init__(self, config: dict):
        ...
        self.min_confidence = config.get('presence_confidence_threshold', 0.7)
    ...
    def should_send_alert(self, camera_name: str, confidence: float) -> bool:
        ...
        if confidence < self.min_confidence:
            LOGGER.debug(f"Presence confidence {confidence:.3f} below threshold {self.min_confidence}")
            return False
    ...
    async def save_image_locally(self, image: ViamImage, person_id: str) -> str:
        ...
        try:
            ...
            filename = f"after_hours_detection_{person_id}_{timestamp}.jpg"
            ...
            LOGGER.info(f"📸 After hours detection image saved: {image_path}")
            return image_path
    ...
    def format_alert_message(self, 
        ...
        message = f"🚨 PERSON DETECTED AFTER HOURS 🚨\n"
        message += f"Camera: {camera_name}\n"
        # message += f"Person: {person_id}\n"
        message += f"Presence Confidence: {confidence:.1%}\n"
    ...
    async def send_after_hours_alert(self, 
                            camera_name: str,
                            alert_type: str,
                            person_id: str, 
                            confidence: float,
        ...
        """Send after-hours detection alert via Twilio SMS (file-fallback only)."""
        ...
        try:
            ...
            await self.save_after_hours_image(camera_name, person_id, confidence, image, detection_info=None, keypoints=keypoints)
    ...
    async def send_webhook_notification(self, camera_name: str, person_id: str, confidence: float, timestamp: datetime, metadata: Optional[Dict[str, Any]] = None, image: Optional[ViamImage] = None, alert_type: str = "after_hours") -> bool:
        ...
        try:
            ...
            primary_payload = {
                ...
                "confidence": confidence,
                "severity": "critical",
                "title": "After Hours Presence Alert Detected",
                "message": f"After hours presence detected on {camera_name} with {confidence:.1%} confidence",
            ...
            if image:
                try:
                    ...
                    primary_payload["image_filename"] = f"after_hours_alert_{camera_name}_{timestamp.strftime('%Y%m%d_%H%M%S')}.jpg"
            ...
            rigguardian_payload = {
                ...
                "title": "🚨 After Hours Alert - Immediate Action Required",
                "message": f"After hours presence detected on {camera_name} with {confidence:.1%} confidence",
                "requires_immediate_attention": True,
            ...
            minimal_payload = {
                ...
                "message": "After hours presence detected",
                "requires_immediate_attention": True,
                "notification_type": "web_push"
            }
    ...
    async def save_after_hours_image(self, camera_name: str, person_id: str, confidence: float, image: ViamImage, detection_info=None, keypoints=None):
        """Save after hours detection image using the file-based fallback only."""

        try:
            LOGGER.info(f"🔄 Saving after hours image (file fallback) for camera: {camera_name}")
            ...
            file_result = await self._save_after_hours_image_to_file(camera_name, person_id, confidence, image, keypoints=keypoints)

            # Wait asynchronously for 1 minute to ensure _save_after_hours_image_to_file has completed
            ...
            if isinstance(file_result, dict) and 'filename' in file_result:
                ...
                if detection_info:
                    os.environ['AFTER_HOURS_IMAGE_FILENAME'] = filename  # Pass the filename via environment variable
                    await annotate_main(detection_info)

            return file_result
        
        except Exception as e:
            LOGGER.error(f"❌ Error in save_after_hours_image: {e}")
            LOGGER.error(traceback.format_exc())
            return {"status": "error", "method": "save_after_hours_image", "error": str(e)}

    async def _save_after_hours_image_to_file(self, camera_name: str, person_id: str, confidence: float, image: ViamImage, keypoints=None):
        ...
        try:
            ...
            # Create metadata file with After Hours tag for data manager to process
            ...
            if os.path.exists(filepath):
                ...
                LOGGER.info(f"✅ After hours image saved: {filename} ({file_size} bytes)")
                LOGGER.info(f"📋 Metadata saved: {metadata_filename}")
                LOGGER.info(f"🎯 Component: {camera_name}, Tags: ['After_Hours']")
